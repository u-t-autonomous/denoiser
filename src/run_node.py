...
def callback(msg):
    global err, log, logdf, time_steps, x_size, y_size, target, table, steps, lock_80, lock_90, lock_95, trial, trials, name, pub, new_state, target_state
    ...
    if time_steps == steps:
        ...
        ts_log = [z for z,y,x,w in log]
        log = []
        logdf['states'] += states
        logdf['actions'] += actions
        logdf['ts'] += ts_log
        logdf['trial'] += [trial] * len(ts_log)
        trial += 1
    ...
    all_K = det_all_k(s, states, new_actions)
